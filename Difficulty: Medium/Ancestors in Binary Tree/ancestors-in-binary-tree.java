...
class Solution
{
    ...
    public static ArrayList<Integer> Ancestors(Node root, int target)
    {
        ...
        addToList(root,arr,target);
        return arr; 
    }
    public static boolean addToList(Node root,ArrayList<Integer> ar,int target){
        if(root==null){
            return false; 
        }
        if(root.data==target){
        ...
        if(addToList(root.left,ar,target) || addToList(root.right,ar,target)){
            ar.add(root.data);
            return true; 
        }
        return false; 
