...
class Solution {
  public:
    Node* codeGeeks(vector<int>& nums,int left,int right) {
        // Code here
        if(left > right)
        {
            return NULL;
        ...
        node->left=codeGeeks(nums,left,mid-1);
        node->right=codeGeeks(nums,mid+1,right);
        
    }
    Node* sortedArrayToBST(vector<int>& nums)
    {
        Node* root = codeGeeks(nums,0,nums.size()-1);
        return root;
    }
};
